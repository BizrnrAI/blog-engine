    ...
    return {
        async releaseReviewedPost({ slug, expectedRevision, publishedAt }) {
            ...
            if (structuralErrors.length)
                throw validationError(cleanSlug, structuralErrors);
            if (options.validatePost) {
                ...
                if (errors.length)
                    throw validationError(cleanSlug, errors);
            }
        ...
        while (true) {
            ...
            for (const row of rows) {
                if (String(row.site_id || '') !== siteId)
                    throw new Error('AllWeb reviewer tenant violation in blog_list');
            ...
            if (!Number.isInteger(next) || next <= offset)
                throw new Error('AllWeb reviewer: invalid blog_list pagination');
            offset = next;
        }
        return slugs;
    }
}
function reviewedPost(row, siteId, revision) {
    return {
        siteId,
        status: 'review',
        revision,
        ...
        faqs: Array.isArray(row.faqs)
            ? row.faqs.map((faq) => ({ q: String(faq?.question ?? faq?.q ?? ''), a: String(faq?.answer ?? faq?.a ?? '') }))
            : [],
...
function validationError(slug, errors) {
