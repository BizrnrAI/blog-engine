...
export function createSupabaseBlogReader(options) {
    const url = new URL(options.url);
    if (!/^https?:$/.test(url.protocol) || !options.anonKey || !options.siteId)
        throw new Error('Supabase reader requires url, anonKey, and siteId');
    const table = options.table || 'blog_posts';
    const schema = options.schema || 'public';
    if (![table, schema].every((value) => /^[a-z_][a-z0-9_]*$/i.test(value)))
        throw new Error('Invalid Supabase table/schema');
    ...
    async function read(extra) {
        ...
        if (!r.ok)
            throw new Error(`Supabase published-post read HTTP ${r.status}`);
        const rows = await r.json();
        if (!Array.isArray(rows))
            throw new Error('Invalid Supabase published-post response');
        return rows.map((row) => {
            if (row.site_id !== options.siteId || row.status !== 'published' || !row.published_at || String(row.published_at).slice(0, 10) > today)
                throw new Error('Supabase returned an unpublished or foreign post');
    ...
    return {
        ...
        async getPublishedPost(slug) {
            ...
            if (rows[0] && rows[0].slug !== slug)
                throw new Error('Supabase returned a different slug');
            return rows[0] || null;
        },
    };
}
//# sourceMappingURL=supabase-reader.js.map