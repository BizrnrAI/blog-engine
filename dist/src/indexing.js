...
export async function waitUntilBlogUrlsLive(urls, timeoutMs = 60_000) {
...
export async function indexingToken() {
...
export async function pingIndexNow(urls) {
    ...
    try {
        ...
        if (!r.ok)
            throw new Error(`IndexNow rejected submission: HTTP ${r.status}`);
    }
    catch (err) {
