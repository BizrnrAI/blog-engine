...
export function classifyQueryIntent(query) {
    ...
    if (custom)
        return custom(query);
    if (VERIFICATION_RE.test(query))
        return 'verification';
    if (COMMERCIAL_RE.test(query))
        return 'commercial';
 ...
 */
export function cannibalizationPairs(rows, minImpressions = 10) {
    ...
    for (const [query, pages] of byQuery) {
        ...
        if (competing.length >= 2)
            out.push({ query, pages: competing.sort((a, b) => b.impressions - a.impressions) });
    }
