...
function resolve(options) {
    ...
    if (!/^https?:$/.test(new URL(url).protocol))
        throw new Error('Supabase store: invalid URL');
    if (!/^[a-z_][a-z0-9_]*$/i.test(options.table || 'blog_posts') || !/^[a-z_][a-z0-9_]*$/i.test(options.schema || 'public'))
        throw new Error('Supabase store: invalid table/schema identifier');
    return {
        url,
        key,
...
export function createSupabaseStore(options) {
    ...
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 120_000)
        throw new Error('Supabase store: invalid timeoutMs');
    if (!Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 1000)
        throw new Error('Supabase store: invalid pageSize');
    const headers = {
        apikey: cfg.key,
    ...
    return {
        ...
        async listPosts() {
            ...
            while (true) {
                ...
                if (!r.ok)
                    throw new Error(`Supabase list HTTP ${r.status}`);
                const rows = await r.json();
                if (!Array.isArray(rows))
                    throw new Error('Supabase list returned an invalid corpus');
                for (const row of rows) {
                    if (row.site_id !== cfg.siteId)
                        throw new Error('Supabase list returned a foreign tenant');
                    if (!options.includeDrafts && row.status !== 'published')
                        throw new Error('Supabase list returned a non-published row');
                    if (seen.has(row.slug))
                        throw new Error('Supabase pagination repeated a post');
                    seen.add(row.slug);
        ...
        async putPost({ post, cover, markdown, dateISO, isRefresh }) {
            ...
            if (isRefresh && options.publishStatus === 'draft')
                throw new Error('Supabase refresh cannot unpublish a live row');
            ...
            if (!r.ok)
                throw new Error(`Supabase ${isRefresh ? 'refresh' : 'insert'} HTTP ${r.status}`);
            if (isRefresh) {
        ...
        async putAsset(key, data, contentType) {
            if (key.includes('://') || key.split('/').some((part) => part === '..' || part === '.'))
                throw new Error('Supabase asset key must be a relative object path');
