 ...
 */
export function createResilientAllWebBlogReader(options) {
    const lastGoodLists = new Map();
    const lastGoodPosts = new Map();
    let lastOkAt = null;
    ...
    return {
        async listPublishedPosts({ includeContent = false, force = false } = {}) {
            ...
            try {
                ...
                if (cached)
                    return { posts: cached, available: true, stale: true };
        ...
        async getPublishedPost(slug, { force = false } = {}) {
            try {
                ...
                if (post)
                    lastGoodPosts.set(slug, post);
                else
                    lastGoodPosts.delete(slug);
                recordSuccess();
                ...
                if (cached)
                    return { post: cached, available: true, stale: true };
...
//# sourceMappingURL=allweb-resilient-reader.js.map