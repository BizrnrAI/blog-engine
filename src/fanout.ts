...
export function questionLikeQueries(queries: readonly { query: string; impressions: number }[], limit = 12): string[] {
  return [...queries]
...
export function validateFanout(passages: FanoutPassage[], expected: number): string[] {
  ...
  passages.forEach((p, i) => {
    ...
    if (w < 30 || w > 80) errs.push(`passage ${i + 1}: answer must be 40-60 words (got ${w})`);
  });
...
export async function generateFanoutPassages(
  ownerPath: string,
  options: { queries?: string[]; count?: number } = {},
  ...
  if (!queries.length) throw new Error(`fanout: no question-like queries for ${ownerPath} (pass --queries or configure Search Console)`);
