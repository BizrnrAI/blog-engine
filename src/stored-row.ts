...
export function storedRowToPost(row: Record<string, any>): ParsedBlogPost {
  ...
  return {
    ...
    faqs: Array.isArray(row.faqs)
      ? row.faqs.map((faq: any) => ({ question: String(faq?.question ?? faq?.q ?? ''), answer: String(faq?.answer ?? faq?.a ?? '') }))
      : [],
