...
const DEFAULT_RULES: Required<Omit<BlogContentRules, 'blockedPhrases'>> & { blockedPhrases: readonly string[] } = {
  minBodyWords: 450,
...
export function contentRules(): typeof DEFAULT_RULES {
...
function blockquoteWordCount(body: string): number {
...
export function validateGeneratedPost(
  ...
  else if (post.description.length > rules.maxDescriptionChars) errs.push(`description > ${rules.maxDescriptionChars} chars even for clamping`);
