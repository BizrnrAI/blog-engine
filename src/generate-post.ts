...
async function callLLM(messages: Array<{ role: string; content: string }>): Promise<Record<string, unknown>> {
...
export function validateGeneratedPost(
  post: Partial<GeneratedBlogPost>,
  args: { existingSlugs: string[]; topic: SeoTopic },
): string[] {
  const errs: string[] = [];
  ...
  if (!post.description || post.description.length < 80 || post.description.length > 175) errs.push('description must be 80-175 chars');
  if (!post.category || !ALLOWED_CATEGORIES.includes(post.category)) errs.push('category not allowed: ' + post.category);
  ...
  if (!post.body || wordCount(post.body) < 450) errs.push(`body too short (>= 450 words; got ${wordCount(post.body || '')})`);
  if ((post.body || '').match(/^#\s/m) || (post.body || '').match(/\n#\s/)) errs.push('body must not contain an H1 (#)');
  const h2 = ((post.body || '').match(/^##\s/gm) || []).length;
  if (h2 < 3) errs.push(`body needs >= 3 H2 sections (got ${h2})`);
  ...
  if (args.topic.mustBacklink && !(post.body || '').includes(backlinkHost)) errs.push(`cross-promo post must link ${backlinkHost}`);
...
function normalizePost(raw: Record<string, unknown>): GeneratedBlogPost {
  ...
  return {
    ...
    faqs: Array.isArray(raw.faqs)
      ? raw.faqs.map((f) => ({ q: String((f as any).q || ''), a: String((f as any).a || '') }))
      : [],
