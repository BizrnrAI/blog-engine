...
export function hasEmDash(value: unknown): boolean {
...
export function normalizeBlogProse(text: string): string {
...
export function assertNoEmDashes(value: unknown): void {
  if (hasEmDash(value)) throw new Error('Blog posts must never contain em dashes, including HTML entities');
}
