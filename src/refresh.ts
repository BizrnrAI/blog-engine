...
async function callLLM(messages: Array<{ role: string; content: string }>): Promise<string> {
  ...
  if (hook) return hook({ messages, text: BLOG_CONFIG.text });
...
export async function refreshBlogPost(
  ...
  const frontmatter: Record<string, string> = store.root && existsSync(file) ? parseBlogFrontmatter(readFileSync(file, 'utf8')).frontmatter : {};
  ...
  const topic: SeoTopic = { type: 'editorial', keyword: stored.title || slug, category: stored.category || '', angle: 'refresh', mustBacklink: false, slug };
  ...
  if (!post) throw new Error(`Could not refresh ${slug} after 3 attempts: ` + errs.join('; '));
  ...
  if (!args.dryRun) {
    ...
    if (persist) {
      ...
      if (alsoWriteFile) writeFileSync(file, markdown, 'utf8');
    } else {
...
export async function refreshBlogRun(root: string, options: RefreshRunOptions): Promise<RefreshRunResult> {
  const logPrefix = BLOG_CONFIG.logPrefix || '[blog-engine]';
  if (process.env.BLOG_ENGINE_DISABLED === '1') return { refreshed: [], candidates: [], skipped: 'BLOG_ENGINE_DISABLED' };
  ...
  if (cooldown > 0 && !(options.slugs && options.slugs.length)) {
    ...
    if (held.length) console.log(`${logPrefix} refresh: ${held.join(', ')} refreshed within ${cooldown}d - holding for the evidence window`);
