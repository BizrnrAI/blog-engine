...
export function blogCacheControl(stale = false): string {
  return stale ? BLOG_NO_STORE : BLOG_CACHE_CONTROL;
}

/** Framework-neutral retryable response for a canonical-store outage. */
export function blogUnavailableResponse({
  body = 'The publishing store is temporarily unavailable. Please retry shortly.',
  contentType = 'text/plain; charset=utf-8',
  retryAfterSeconds = BLOG_RETRY_AFTER_SECONDS,
}: {
  body?: string;
  contentType?: string;
