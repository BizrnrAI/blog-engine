 ...
 */
export function validateBlogEngineRuntime(runtime: BlogEngineRuntime): string[] {
  const problems: string[] = [];
  const config = runtime?.config;
  const topics = runtime?.topics;

  if (!config) return ['config is missing — pass { config, topics, brandPersona }'];
  if (!topics) problems.push('topics is missing — pass { config, topics, brandPersona }');
  ...
  if (!id) problems.push('identity is missing');
  else {
    if (!id.name?.trim()) problems.push('identity.name is required (the brand name)');
    ...
    if (!id.siteHost?.trim()) problems.push('identity.siteHost is required (e.g. "example.com", no scheme)');
    else if (/^https?:\/\//.test(id.siteHost)) problems.push('identity.siteHost must be a bare host, without https://');
    if (!id.agent?.name?.trim()) problems.push('identity.agent.name is required (use the brand name if there is no named person)');
    if (id.backlink && !isHttpUrl(id.backlink.url)) {
      problems.push('identity.backlink.url must be an absolute http(s) URL');
    }
    if (id.ctaPath && !id.ctaPath.startsWith('/')) problems.push('identity.ctaPath must be a site-relative path starting with "/"');
  }

  /* --------------------------------------------------------------------- paths --- */
  const paths = config.paths;
  if (!paths) problems.push('paths is missing');
  else {
    for (const key of ['blogDir', 'assetDir', 'heroDir', 'brandLogo', 'watermarkLogo'] as const) {
      if (!paths[key]?.trim()) problems.push(`paths.${key} is required`);
    }
  }
  ...
  if (topics) {
    ...
    if (!topics.internalLinks?.length) {
      ...
      if (bad.length) problems.push(`topics.internalLinks must be site-relative paths starting with "/" (got ${bad.slice(0, 3).join(', ')})`);
    }
    if (!topics.gradients?.length) problems.push('topics.gradients must contain at least one value (used to theme cards)');
    ...
    if (strayEditorial.length) {
      problems.push(
        `topics.editorial has categories missing from allowedCategories: ${[...new Set(strayEditorial.map((t) => t.category))].join(', ')}`,
      );
    }
  }

  /* ---------------------------------------------------------------------- misc --- */
  if (!config.rss?.path?.startsWith('/')) problems.push('rss.path must be a site-relative path starting with "/"');
  if (config.gsc && !config.gsc.property?.trim()) problems.push('gsc.property is required (e.g. "sc-domain:example.com")');
...
export function assertBlogEngineRuntime(runtime: BlogEngineRuntime): void {
  const problems = validateBlogEngineRuntime(runtime);
  if (problems.length) throw new BlogEngineConfigError(problems);
}
