 ...
 */
export const DEFAULT_FRONTMATTER_ALIASES: Readonly<Record<string, string>> = Object.freeze({
  pubDate: 'date',
  publishDate: 'date',
  updatedDate: 'updated',
 ...
 */
export function coerceFrontmatterValue(canonicalKey: string, value: string): string {
...
export function resolveFrontmatterAliases(): Readonly<Record<string, string>> {
  try {
...
export function normalizeFrontmatter(
  ...
  aliases: Readonly<Record<string, string>> = resolveFrontmatterAliases(),
): Record<string, string> {
  const out: Record<string, string> = { ...frontmatter };
  ...
  for (const key of ['readMins', 'date', 'updated']) {
    if (out[key] !== undefined) out[key] = coerceFrontmatterValue(key, out[key]);
  }
  return out;
}
